...
This folder implements the construction plan: runnable **IronPython** scripts for Dynamo for Civil 3D, sample **CSV** inputs, **`project.json`** for M8, and documentation.

**Start here for a single narrative:** [road_automation_user_guide.html](../road_automation_user_guide.html) (installation, steps, manual vs automated).

**New users:** [road_automation_beginners.html](../road_automation_beginners.html) (concepts in plain language, glossary, error hints).
...
2. **New full copy of this folder:** run `python tools/clone_new_job.py --dest "…\YourJob\civil3d_automation"` (see `tools/README.md`), or copy `config/project.example.json` to `config/project.json` and copy header-only files from `csv/templates/` into `csv/`. Optionally run `tools/run_preflight.bat`.
...
5. Run the **marking** graph from `road_marking_dynamo.html` for each source line.
...
- [road_automation_beginners.html](../road_automation_beginners.html) — concepts, glossary, error hints  
- [road_marking_dynamo.html](../road_marking_dynamo.html) — marking module + pipeline overview  
- [road_automation_advanced.html](../road_automation_advanced.html) — extended playbook narrative

## Testing

From the `civil3d_automation` folder run [`run_tests.bat`](run_tests.bat) (or `python -m unittest discover -s tools -p "test_*.py" -v`). Requires Python 3; optional tests use **openpyxl** (`pip install -r requirements-tools.txt`).
